...
public class FbiBeanUtils {
    ...
    public static void copyFieldsFromResultSet(ResultSet resultSet, Object des) throws SQLException {
        Field[] srcFields = des.getClass().getDeclaredFields();
        for (Field field : srcFields) {
            field.setAccessible(true);
            String value = resultSet.getString(field.getName().toLowerCase());
            try {
                if (value != null)
                    field.set(des, value);
                else {
                ...
                throw new RuntimeException(field.getName() + "字段赋值出现异常...值类型为String...");
            }
        }
    }

    public static void copyFieldsIgnoreCase(Object src, Object des) throws IllegalAccessException {
        ...
        for (Field field : srcFields) {
            ...
            if (desFieldTypeName.equals("String")) {
                if (fieldValue != null) {
                    desField.set(des, fieldValue.toString());
                } else {
                    desField.set(des, "");
                }
